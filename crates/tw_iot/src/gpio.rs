...
mod tests {
    ...
    #[test]
    fn test_simulator_connect() {
        let mut mgr = GpioManager::new(Board::Simulator);
        assert!(mgr.connect().is_ok());
        assert!(mgr.connected);
    }

    #[test]
    fn test_digital_write_read() {
        let mut mgr = GpioManager::new(Board::Pico);
        mgr.connect().unwrap();
        mgr.pin_mode(0, PinMode::Output).unwrap();
        mgr.digital_write(0, true).unwrap();
        assert_eq!(mgr.pins[&0].state, PinState::High);
        mgr.digital_write(0, false).unwrap();
        assert_eq!(mgr.pins[&0].state, PinState::Low);
    }

    #[test]
    fn test_input_toggle() {
        let mut mgr = GpioManager::new(Board::Simulator);
        mgr.connect().unwrap();
        mgr.pin_mode(5, PinMode::Input).unwrap();
        assert!(!mgr.digital_read(5).unwrap());
        mgr.sim_toggle(5);
        assert!(mgr.digital_read(5).unwrap());
    }
    ...
    fn test_pwm() {
        let mut mgr = GpioManager::new(Board::Pico);
        mgr.connect().unwrap();
        mgr.pin_mode(2, PinMode::Pwm).unwrap();
        mgr.pwm_write(2, 0.75).unwrap();
        assert!((mgr.pins[&2].value - 0.75).abs() < 0.001);
    }

    #[test]
    fn test_wrong_mode_errors() {
        let mut mgr = GpioManager::new(Board::Simulator);
        mgr.connect().unwrap();
        mgr.pin_mode(0, PinMode::Input).unwrap();
        assert!(mgr.digital_write(0, true).is_err());
    }

    #[test]
    fn test_invalid_pin() {
        let mut mgr = GpioManager::new(Board::Pico);
        assert!(mgr.pin_mode(99, PinMode::Output).is_err());
    }
    ...
    fn test_reset() {
        ...
        mgr.digital_write(0, true).unwrap();
        mgr.reset();
        assert_eq!(mgr.pins[&0].mode, PinMode::Unset);
        assert_eq!(mgr.pins[&0].state, PinState::Unknown);
    }
}
