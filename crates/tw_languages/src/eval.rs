...
mod tests {
    ...
    fn eval(expr: &str) -> f64 {
        ...
        let str_vars: HashMap<String, String>    = HashMap::new();
        let arrays: HashMap<String, Vec<f64>>    = HashMap::new();
        evaluate(expr, &vars, &str_vars, &arrays).unwrap()
    }
    ...
    fn arithmetic() {
        ...
        assert_eq!(eval("10 MOD 3"), 1.0);
    }

    #[test]
    fn functions() {
        assert!((eval("SQRT(4)") - 2.0).abs() < 1e-9);
        assert!((eval("ABS(-5)") - 5.0).abs() < 1e-9);
    }
    ...
    fn variables() {
        ...
        let earr: HashMap<String, Vec<f64>> = HashMap::new();
        let v = evaluate("2 * X + 5", &vars, &empty, &earr).unwrap();
        assert_eq!(v, 25.0);
    }
}
